...
class Box(Problem):
    ...
    def goal_test(self, state):
        _, box1, box2 = state
        return box1 == self.end_point and box2 == self.end_point
    ...
    def check_valid(state):
        ...
        if manx < 0 or manx > 4 or many < 0 or many > 4:
            return False
        if box1x < 0 or box1x > 4 or box1y < 0 or box1y > 4:
            return False
        if box2x < 0 or box2x > 4 or box2y < 0 or box2y > 4:
            return False

        if box1x == box2x and box1y == box2y:
            if not (box1x == 4 and box1y == 4):
                return False
    ...
    def successor(self, state):
        ...
        actions = ['Move man up', 'Move man down', 'Move man left', 'Move man right']
        directions = [(0, 1), (0, -1), (-1, 0), (1, 0)]

        for action, (dx, dy) in zip(actions, directions):
            nmanx, nmany = manx + dx, many + dy

            if (nmanx, nmany) != box1 and (nmanx, nmany) != box2:
                new_state = ((nmanx, nmany), box1, box2)
                if self.check_valid(new_state):
                    successor[action] = new_state

            elif (nmanx, nmany) == box1:
                new_box = (box1[0] + dx, box1[1] + dy)
                if new_box != box2 or new_box == self.end_point:
                    new_state = ((nmanx, nmany), new_box, box2)
                    if self.check_valid(new_state):
                        successor[f"Push box 1 {action.split()[-1]}"] = new_state

            elif (nmanx, nmany) == box2:
                new_box = (box2[0] + dx, box2[1] + dy)
                if new_box != box1 or new_box == self.end_point:
                    new_state = ((nmanx, nmany), box1, new_box)
                    if self.check_valid(new_state):
                        successor[f"Push box 2 {action.split()[-1]}"] = new_state
