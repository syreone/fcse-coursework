...
def parse_state(line):
    ...
    for segment in line.strip().split(";"):
        ...
        if segment == "":
            ...
            pillars.append(tuple(int(x) for x in segment.split(",")))
...
class Pillars(Problem):
    ...
    def result(self, state, action):
        i, j = action
        pillars = [list(p) for p in state]
        disk = pillars[i].pop()
        pillars[j].append(disk)
        return tuple(tuple(p) for p in pillars)
...
if __name__ == "__main__":
    initial = parse_state(input())
    goal = parse_state(input())

    problem = Pillars(initial, goal)
    ...
    if node is None:
        ...
        print([f"MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}" for i, j in solution])