...
enum PizzaType {
    Standard(0), Pepperoni(1), Extra_cheese(2), Vegeterian(3);
    private int value;
    private int cost;
...
class Order {
    ...
    public void addItems(Item item, int count) throws OrderLockedException, ItemOutOfStockException {
        if (locked) {
        ...
        Optional<OrderItem> orderItem = items.stream().filter(each -> each.getItem().getType().equals(item.getType())).findFirst();
    ...
    public void displayOrder() {
        IntStream.range(0, items.size())
                .forEach(i -> { OrderItem order = items.get(i);
                    System.out.printf("%3d.% -15 sx %2d%5 d$\n", i + 1, order.getItem().getType(), order.getCount(), order.getPrice());
                });
        System.out.printf("% -22s%5 d$\n", " Total :", getPrice());
    }
