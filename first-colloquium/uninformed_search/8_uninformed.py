...
class DiskProblem(Problem):
    def __init__(self, n, l):
        initial = tuple(range(1, n + 1)) + tuple([0] * (l - n))
        goal = tuple([0] * (l - n)) + tuple(range(n, 0, -1))
    ...
    def actions(self, state):
        ...
        for i in range(self.l):
            ...
            if disk == 0:
                continue
            if i + 1 < self.l and state[i + 1] == 0:
                actions_list.append(f"R1: Disk {disk}")
            if i + 2 < self.l and state[i + 2] == 0 and state[i + 1] != 0:
                actions_list.append(f"R2: Disk {disk}")
            if i - 1 >= 0 and state[i - 1] == 0:
                actions_list.append(f"L1: Disk {disk}")
            if i - 2 >= 0 and state[i - 2] == 0 and state[i - 1] != 0:
                actions_list.append(f"L2: Disk {disk}")
        return actions_list

    def result(self, state, action):
        kind = action.split(":")[0]
        disk_val = int(action.split(" ")[2])
        curr = state.index(disk_val)
        ...
        new_state[curr], new_state[new_pos] = new_state[new_pos], new_state[curr]
