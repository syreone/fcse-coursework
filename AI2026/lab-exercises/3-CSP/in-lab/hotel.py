...
# gostite mozhe da odbijat soba ako kapacitetot na sobata e pogolem od nivnoto pobaruvanje, za toa se koristi parametar
# exact=True ako gostinot ne prifakja pogolemi sobi od negovoto baranje,  False ako gostinot prifakja.
...
def read_input():
    ...
    for _ in range(num_families):
        parts = input().split()
        name, size, reqs_string = parts[0], parts[1], parts[2]
        exact = len(parts) > 3 and parts[3] == 'exact=True'
    ...
    for _ in range(num_rooms):
        ...
        rooms[int(room_id)] = {'floor': int(room_id[0]), 'capacity': int(capacity), 'amenities': amenities}
...
if __name__ == '__main__':
    ...
    for fname, family in families.items():
        valid_rooms[fname] = [
            ...
            if (room['capacity'] == family['size'] if family['exact'] else room['capacity'] >= family['size'])
