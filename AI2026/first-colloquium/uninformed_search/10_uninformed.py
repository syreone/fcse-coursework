...
def parse_input(n, fields):
    flat = list(map(int, fields.split(',')))
    return tuple(tuple(flat[i * n:(i + 1) * n]) for i in range(n))

class Board(Problem):
    def __init__(self, initial, n):
        self.n = n
        goal = tuple(tuple(1 for _ in range(n)) for _ in range(n))
...
if __name__ == "__main__":
    ...
    if result is None:
        ...
        print([f'x: {r}, y: {c}' for r, c in result.solution()])
