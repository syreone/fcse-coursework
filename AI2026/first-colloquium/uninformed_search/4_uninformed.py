...
class Topchinja(Problem):
    def __init__(self, initial, N, obstacles):
        super().__init__(initial)
        self.N = N
        self.obstacles = obstacles
        self.goal_pos = (N // 2, N - 1)

    def goal_test(self, state):
        return len(state) == 1 and state[0] == self.goal_pos
    ...
    def successor(self, state):
        ...
        for x, y in balls:
            for dir_name, (dj_x, dj_y), (dl_x, dl_y) in directions:
                jx, jy = x + dj_x, y + dj_y
                lx, ly = x + dl_x, y + dl_y

                if (jx, jy) not in balls:
                    continue
                if not (0 <= lx < self.N and 0 <= ly < self.N):
                    continue
                if (lx, ly) in balls or (lx, ly) in self.obstacles:
                    continue

                new_balls = set(balls)
                new_balls.remove((x, y))
                new_balls.remove((jx, jy))
                new_balls.add((lx, ly))

                successors[f"{dir_name}: (x={x},y={y})"] = tuple(sorted(new_balls))
...
if __name__ == '__main__':
    N = int(input())
    num_balls = int(input())
    balls = [tuple(map(int, input().split(','))) for _ in range(num_balls)]

    num_obstacles = int(input())
    obstacles = set(tuple(map(int, input().split(','))) for _ in range(num_obstacles))

    initial_state = tuple(sorted(balls))
    problem = Topchinja(initial_state, N, obstacles)
