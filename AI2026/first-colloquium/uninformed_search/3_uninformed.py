...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i, (x, y) in enumerate(state):
            for action_name, (dx, dy) in zip(actions, directions):
                new_pos = (x + dx, y + dy)
                new_state = list(state)
                new_state[i] = new_pos
                new_state_tuple = tuple(new_state)
                if self.check_valid(new_state_tuple):
                    successor[f'Move square {i + 1} {action_name}'] = new_state_tuple
...
if __name__ == '__main__':
    ...
    problem = Squares(initial_state, goal_state)
