...
OBSTACLES = {
    (4, 6), (5, 1), (9, 8), (0, 8), (1, 3), (1, 9), (6, 2), (4, 5), (3, 9),
    (5, 6), (3, 6), (9, 7), (9, 4), (8, 8), (1, 2), (6, 1), (4, 1), (4, 7),
    (8, 4), (8, 1), (8, 7), (0, 9), (1, 4), (0, 6), (2, 9), (6, 0), (6, 9)
}
...
class Pacman(Problem):
    ...
    # zadacata e reshena megjutoa nema da izleze zelena bidejki bfs ne e sekogash isto
    # dolzhinata na rezultatot e ista kako taa na courses shto znachi najverojatno e validna
    def successor(self, state):
        successors = {}
        x, y, direction, remaining = state

        dx, dy = DIRECTIONS[direction]
        nx, ny = x + dx, y + dy
        if 0 <= nx < 10 and 0 <= ny < 10 and (nx, ny) not in OBSTACLES:
            new_remaining = tuple(d for d in remaining if d != (nx, ny))
            successors["Move forward"] = (nx, ny, direction, new_remaining)

        nx, ny = x - dx, y - dy
        if 0 <= nx < 10 and 0 <= ny < 10 and (nx, ny) not in OBSTACLES:
            new_remaining = tuple(d for d in remaining if d != (nx, ny))
            successors["Move backward"] = (nx, ny, direction, new_remaining)
        ...
        if 0 <= nx < 10 and 0 <= ny < 10 and (nx, ny) not in OBSTACLES:
            new_remaining = tuple(d for d in remaining if d != (nx, ny))
            successors["Turn left"] = (nx, ny, new_dir, new_remaining)
        ...
        if 0 <= nx < 10 and 0 <= ny < 10 and (nx, ny) not in OBSTACLES:
            new_remaining = tuple(d for d in remaining if d != (nx, ny))
            successors["Turn right"] = (nx, ny, new_dir, new_remaining)
...
if __name__ == '__main__':
    ...
    remaining = tuple(sorted(d for d in dots if d != (x, y)))
    initial = (x, y, direction, remaining)
